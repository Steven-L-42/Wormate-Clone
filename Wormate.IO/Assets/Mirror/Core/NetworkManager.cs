...
namespace Mirror
{
    ...
    public class NetworkManager : MonoBehaviour
    {
        ...
        [Tooltip("Soll das Network Manager-Objekt bei Szenenänderungen beibehalten werden?")]
        ...
        [Tooltip("Multiplayer-Spiele sollten immer im Hintergrund laufen, damit das Netzwerk nicht abläuft.")]
        public bool runInBackground = true;

        /// <summary>Should the server auto-start when 'Server Build' is checked in build settings</summary>
        [Header("Kopflose Builds")]
        [Tooltip("Sollte der Server automatisch starten, wenn „Server Build“ in den Build-Einstellungen aktiviert ist")]
        [FormerlySerializedAs("startOnHeadless")]
        public bool autoStartServerBuild = true;

        [Tooltip("Verbinden Sie den Client automatisch in Headless-Builds. Nützlich für CCU-Tests mit Bot-Clients.\n\nAdresse kann als Kommandozeilenargument übergeben werden.\n\nStellen Sie sicher, dass nur 'autostartServer' oder 'autoconnectClient' aktiviert ist, nicht beide!")]
        ...
        [Tooltip("Server- und Client-Senderate pro Sekunde. Verwenden Sie etwa 60 Hz für schnelle Spiele wie Counter-Strike, um die Latenz zu minimieren. Verwenden Sie etwa 30 Hz für Spiele wie WoW, um die Berechnungen zu minimieren. Verwenden Sie etwa 1-10 Hz für langsame Spiele wie EVE.")]
        [FormerlySerializedAs("serverTickRate")]
        public int sendRate = 30;
        [Obsolete("NetworkManager.serverTickRate wurde in sendRate umbenannt, da dies jetzt für Server und Client konfiguriert wird.")]
        ...
        [Obsolete("NetworkManager.serverTickInterval wurde aus Konsistenzgründen nach NetworkServer.tickInterval verschoben.")]
        ...
        [Header("Szenenverwaltung")]
        [Scene]
        [FormerlySerializedAs("m_OfflineScene")]
        [Tooltip("Szene, zu der Mirror wechselt, wenn der Client oder Server gestoppt wird")]
        ...
        [Tooltip("Szene, zu der Mirror wechselt, wenn der Server gestartet wird. Clients erhalten eine Szenennachricht, um die aktuelle Szene des Servers zu laden, wenn sie sich verbinden.")]
        public string onlineScene = "";

        // transport layer
        [Header("Netzwerk information")]
        [Tooltip("An dieses Objekt angehängte Transportkomponente, die Server und Client für die Verbindung verwenden")]
        public Transport transport;

        /// <summary>Server's address for clients to connect to.</summary>
        [FormerlySerializedAs("m_NetworkAddress")]
        [Tooltip("Netzwerkadresse, über die sich der Client mit dem Server verbinden soll. Der Server verwendet dies für nichts.")]
        ...
        [Tooltip("Maximale Anzahl gleichzeitiger Verbindungen.")]
        public int maxConnections = 100;

        [Header("Authentication")]
        [Tooltip("An dieses Objekt angehängte Authentifizierungskomponente")]
        ...
        [Header("Player-Objekt")]
        [FormerlySerializedAs("m_PlayerPrefab")]
        [Tooltip("Prefab des Player-Objekts. Prefab muss über eine Network Identity-Komponente verfügen. Kann ein leeres Spielobjekt oder ein vollständiger Avatar sein.")]
        ...
        [Tooltip("Soll Mirror den Player nach Szenenwechsel automatisch spawnen?")]
        public bool autoCreatePlayer = true;

        /// <summary>Where to spawn players.</summary>
        [FormerlySerializedAs("m_PlayerSpawnMethod")]
        [Tooltip("Round Robin oder zufällige Reihenfolge der Auswahl der Startposition")]
        ...
        public virtual void OnValidate()
        {
            ...
            {
                Debug.LogError("NetworkManager - Player Prefab muss über eine NetworkIdentity verfügen.");
                playerPrefab = null;
            }
            ...
            {
                Debug.LogWarning("NetworkManager - Player Prefab sollte nicht zur Liste der registrierten spawnbaren Prefabs hinzugefügt werden ... wurde entfernt.");
        ...
        public virtual void Reset()
        {
            ...
            foreach (NetworkManager manager in transform.root.GetComponentsInChildren<NetworkManager>())
            {
                if (manager != this)
                {
                    Debug.LogError($"{name} eine andere Komponente des Typs erkannt {typeof(NetworkManager)} in seiner Hierarchie auf {manager.name}. Es kann nur einen geben, bitte entfernen Sie einen davon.");
        ...
        public void StartServer()
        {
            ...
            {
                Debug.LogWarning("Server bereits gestartet.");
                return;
            }
        ...
        public void StartClient()
        {
            ...
            {
                Debug.LogWarning("Client bereits gestartet.");
                return;
            }
            ...
            {
                Debug.LogError("Das Feld Netzwerkadresse muss im Manager festgelegt werden");
                return;
            }
        ...
        public void StartHost()
        {
            ...
            {
                Debug.LogWarning("Server oder Client bereits gestartet.");
                return;
            }
        ...
        bool InitializeSingleton()
        {
            ...
            if (dontDestroyOnLoad)
            {
                if (singleton != null)
                {
                    Debug.LogWarning("Mehrere NetworkManager in der Szene erkannt. Es kann immer nur ein NetworkManager vorhanden sein. Der doppelte NetworkManager wird zerstört.");
                    Destroy(gameObject);
        ...
        public virtual void ServerChangeScene(string newSceneName)
        {
            ...
            {
                Debug.LogError("ServerChangeScene leerer Szenenname");
                return;
            }

            if (NetworkServer.isLoadingScene && newSceneName == networkSceneName)
            {
                Debug.LogError($"Szenenwechsel ist bereits im Gange für {newSceneName}");
                return;
            }
        ...
        internal void ClientChangeScene(string newSceneName, SceneOperation sceneOperation = SceneOperation.Normal, bool customHandling = false)
        {
            if (string.IsNullOrWhiteSpace(newSceneName))
            {
                Debug.LogError("ClientChangeScene leerer Szenenname");
                return;
            }
        ...
        public virtual void OnServerAddPlayer(NetworkConnectionToClient conn)
        {
            ...
            //((MonoBehaviour)player.GetComponent("GrowPlayer")).enabled = true;
            //((MonoBehaviour)player.GetComponent("ResizeCameraSize")).enabled = true;
            //((MonoBehaviour)player.GetComponent("Eat")).enabled = true;
            //((MonoBehaviour)player.GetComponent("SnakeManager")).enabled = true;
        }
